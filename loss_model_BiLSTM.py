...
class RouteNet_Fermi(tf.keras.Model):
    def __init__(self):
        ...
        self.z_score = {'traffic': [1385.4059, 859.8119],
        ...
        # BiLSTM Cells
        self.path_update_forward = tf.keras.layers.LSTMCell(self.path_state_dim // 2)
        self.path_update_backward = tf.keras.layers.LSTMCell(self.path_state_dim // 2)
    ...
    def call(self, inputs):
        ...
        for it in range(self.iterations):
            ...
            forward_rnn = tf.keras.layers.RNN(self.path_update_forward, return_sequences=True, return_state=True)
            backward_rnn = tf.keras.layers.RNN(self.path_update_backward, return_sequences=True, return_state=True, go_backwards=True)

            seq_fw, path_state_h_fw, path_state_c_fw = forward_rnn(rnn_input, initial_state=[path_state_h_fw, path_state_c_fw])
            seq_bw, path_state_h_bw, path_state_c_bw = backward_rnn(rnn_input, initial_state=[path_state_h_bw, path_state_c_bw])
            seq_bw = tf.reverse(seq_bw, axis=[1])

            path_state_sequence = tf.concat([seq_fw, seq_bw], axis=2)
            ...
            queue_state_output, [queue_state_h, queue_state_c] = self.queue_update(path_sum, states=[queue_state_h, queue_state_c])
