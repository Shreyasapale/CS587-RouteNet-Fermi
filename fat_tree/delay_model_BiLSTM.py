...
class RouteNet_Fermi(tf.keras.Model):
    def __init__(self):
        ...
        # BiLSTM for path update (using half the dimensions for each direction)
        ...
        # Regular LSTM cells for link and queue updates (same as original)
    ...
    def call(self, inputs):
        ...
        # Split path state for forward and backward processing
        ...
        for it in range(self.iterations):
            ...
            # Store previous path state for later concatenation
            ...
            path_state_sequence_forward, path_state_h_forward, path_state_c_forward = forward_rnn(
                combined_input,
                initial_state=[path_state_h_forward, path_state_c_forward]
            )
            
            # Backward pass through LSTM (processes sequence in reverse)
            backward_rnn = tf.keras.layers.RNN(self.path_backward, 
                                              return_sequences=True, 
                                              return_state=True, 
                                              go_backwards=True)
            path_state_sequence_backward, path_state_h_backward, path_state_c_backward = backward_rnn(
                combined_input,
                initial_state=[path_state_h_backward, path_state_c_backward]
            )
            
            # Reverse the backward sequence to align with forward sequence
            path_state_sequence_backward = tf.reverse(path_state_sequence_backward, axis=[1])
            
            # Concatenate forward and backward sequences
            path_state_sequence = tf.concat([path_state_sequence_forward, path_state_sequence_backward], axis=2)
            
            # Add previous state as first element in sequence
