...
class RouteNet_Fermi(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        for it in range(self.iterations):
            ...
            path_seq_fw, path_h_fw, path_c_fw = forward_rnn(rnn_input, initial_state=[path_h_fw, path_c_fw])
            path_seq_bw, path_h_bw, path_c_bw = backward_rnn(rnn_input, initial_state=[path_h_bw, path_c_bw])

            path_seq_bw = tf.reverse(path_seq_bw, axis=[1])
            path_state_sequence = tf.concat([path_seq_fw, path_seq_bw], axis=-1)

            previous_path_h = tf.concat([path_h_fw, path_h_bw], axis=-1)
        ...
        # Final prediction based on last hidden state
        losses = self.readout_path(previous_path_h)
