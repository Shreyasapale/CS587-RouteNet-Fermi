...
class RouteNet_Fermi(tf.keras.Model):
    def __init__(self):
        ...
        # BiLSTM Cells (forward and backward)
    ...
    def call(self, inputs):
        ...
        # Initialize path hidden states for forward and backward
        ...
        for it in range(self.iterations):
            ...
            path_seq_fw, path_state_h_fw, path_state_c_fw = forward_rnn(rnn_input, initial_state=[path_state_h_fw, path_state_c_fw])
            path_seq_bw, path_state_h_bw, path_state_c_bw = backward_rnn(rnn_input, initial_state=[path_state_h_bw, path_state_c_bw])
            path_seq_bw = tf.reverse(path_seq_bw, axis=[1])

            path_state_sequence = tf.concat([path_seq_fw, path_seq_bw], axis=-1)
            previous_path_h = tf.concat([path_state_h_fw, path_state_h_bw], axis=-1)

            path_state_sequence = tf.concat([tf.expand_dims(previous_path_h, 1), path_state_sequence], axis=1)
