...
class RouteNet_Fermi(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # Initialize path states (split for BiLSTM)
        ...
        for it in range(self.iterations):
            ...
            forward_rnn = tf.keras.layers.RNN(self.path_forward, return_sequences=False, return_state=True)
            backward_rnn = tf.keras.layers.RNN(self.path_backward, return_sequences=False, return_state=True, go_backwards=True)

            _, path_state_h_fw, path_state_c_fw = forward_rnn(rnn_input, initial_state=[path_state_h_fw, path_state_c_fw])
            _, path_state_h_bw, path_state_c_bw = backward_rnn(rnn_input, initial_state=[path_state_h_bw, path_state_c_bw])

            ###################
            #  PATH TO QUEUE  #
            ###################
            current_path_h = tf.concat([path_state_h_fw, path_state_h_bw], axis=-1)
            path_gather = tf.gather(current_path_h, path_to_queue[:, :, 0])
        ...
        # Final output
        final_path_h = tf.concat([path_state_h_fw, path_state_h_bw], axis=-1)
        losses = self.readout_path(final_path_h)
