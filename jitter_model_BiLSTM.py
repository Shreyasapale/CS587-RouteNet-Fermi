...
class RouteNet_Fermi(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        for it in range(self.iterations):
            ...
            path_seq_fw, path_state_h_fw, path_state_c_fw = forward_rnn(combined_input, initial_state=[path_state_h_fw, path_state_c_fw])
            path_seq_bw, path_state_h_bw, path_state_c_bw = backward_rnn(combined_input, initial_state=[path_state_h_bw, path_state_c_bw])
        ...
        jitter = tf.reduce_sum(occupancy_gather / capacity_gather, axis=1)
        return jitter
